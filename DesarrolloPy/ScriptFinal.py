...
df1 = nfv.dfFix(GD_Language,1)
np_array1 = np.array(df1)
df2 = nfv.dfFix(GD_Language,0)
np_array2 = np.array(df2)
np_array3 = np.concatenate((np_array1,np_array2), axis = 1)
...
Camp_Integration = nfv.dfFix(Entities,"GENERAL_INFORMATION:Relationship","GENERAL_INFORMATION:Movement_outside")
nt.mkCSV(Camp_Integration,"Camp_Integration.csv")

Camp_NaturalHazard = nfv.dfFix(Entities,"Enviormental_Issues:Risk:Risk_Flood","Enviormental_Issues:Deforestation")
Camp_NaturalHazard = nt.getSubColumnNames(Camp_NaturalHazard,30)
nt.mkCSV(Camp_NaturalHazard,"Camp_NaturalHazard.csv")

Camp_NaturalHazard_Has_Camp = nfv.dfFix(Entities,"Enviormental_Issues:Risk:Risk_Flood","Enviormental_Issues:Deforestation")
...
Camp_LocalVegetation = nfv.dfFix(Entities,"Enviormental_Issues:Native_Plant","Enviormental_Issues:Native_Crops")
...
Camp_LocalCrop = nfv.dfFix(Entities,"Enviormental_Issues:Native_Crops","Water_table")
Camp_LocalCrop = nt.separateValues(Camp_LocalCrop)
nt.mkCSV(Camp_LocalCrop,"Camp_LocalCrop.csv") #MODIFICAR FOLMULARIO?

df3 = nfv.dfFix(Entities,"Enviormental_Issues:High_enviormental_value","Enviormental_Issues:Native_Plant")
df3 = df3.isin(["yes"])
df4 = nfv.dfFix(Entities,"Enviormental_Issues:Deforestation","Enviormental_Issues:High_enviormental_value")
Camp_Enviroment = nt.concatDF(df3,df4)
nt.mkCSV(Camp_Enviroment,"Camp_Enviroment.csv")

df1 = nfv.dfFix(Bibliography,"Tropical (Write one: Af, Aw or Am)","Temperature")
df1 = df1.transpose()
df1 = df1.dropna()
df1 = df1.transpose()
...
df1 = nfv.dfFix(Entities,"Fuel_Cost:Fuel_Cost_Diesel","ENERGY:Electricity_network")
...
Camp_Mobility = nfv.dfFix(Entities,"GENERAL_INFORMATION:Movement_outside","Population:Women:Infants")
nt.mkCSV(Camp_Mobility,"Camp_Mobility.csv")

Camp_Shelter = nfv.dfFix(Entities,"Shelter:Housing_Improvement","Shelter:Total_shelter")
...
S_Tecknowlege_has_Community = nfv.dfFix(GeneralCitizen,"TICs_Knowledge:Phone_Call","App_USED")
...
FS_Cause = nfv.dfFix(Bibliography,"Cause 1","Affected groups due to food insecurity")
...
df1 = df1.isin(["yes"])
df2 = nfv.dfFix(Bibliography,"Intake (g) - default value 70g-","GENERAL INFORMATION OF REFUGEES SETTLEMENT")
...
FS_FoodAccess_has_Community = nfv.dfFix(GeneralCitizen,"Type_Food:Meat","times:One_time")
...
df1 = nfv.dfFix(Bibliography,"Pork (200 kcal/100g)","Intake (g) - default value 70g-")
